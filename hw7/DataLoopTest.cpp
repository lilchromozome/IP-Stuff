...
struct DataLoopTest {
  ...
  static void OperatorAssignmentTest() {
    ...
    *a = *r;
    ...
    ASSERT(r->start->data == 10);
    ASSERT(r->start->next != nullptr);
    ASSERT(r->start->next->data == 6);
    ASSERT(r->start->next->next != nullptr);
    ASSERT(r->start->next->next->data == 7);  

    ASSERT(a->start != nullptr);
    ASSERT(a->start->data == 10);  // check not changed
    ASSERT(a->start->next != nullptr);
    ASSERT(a->start->next->data == 6);  
    ASSERT(a->start->next->next != nullptr);
    ASSERT(a->start->next->next->data == 7);  
  ...
  static void OperatorConcatenateTest() {
    ...
    *s = *q + *r;
    ...
    ASSERT(q->start->prev->data == 30);
    ASSERT(q->start->prev->prev != nullptr);
    ASSERT(q->start->prev->prev->data == 10);
    ASSERT(q->count == 2);
    ...
    ASSERT(r->start->prev->prev != nullptr);
    ASSERT(r->start->prev->prev->data == 15);
    ASSERT(r->count == 2);
  ...
  static void FunctionSpliceTest() {
    ...
    DataLoop *empty = new DataLoop();
    DataLoop *f = new DataLoop(69);
    ...
    *f = f->splice(*f, 3);
    ...
    ASSERT(f->start != nullptr);
    sss << *f;
    ASSERT(sss.str() == "-> 69 <--> 69 <-");
    std::cout << *f << std::endl;
    sss.str("");

    *empty = empty->splice(*f, 3);
    ...
    *p += 20;
    *p += 25;               //p = 20, 25
    q->splice(*p, 9);       //q = 10, 11, (20, 25), 12, 1, 2, 3, 4
    ASSERT(q->count == 9);
