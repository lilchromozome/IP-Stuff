...
public:
   ...
   *
   * \param[in] num The value to be added to the DataLoop
   */
  TDataLoop(const T & num);
   ...
   */ 
  TDataLoop(const TDataLoop<T> & rhs);
   ...
   */
  TDataLoop<T> & operator=(const TDataLoop<T> & rhs); 
   ...
   */
  bool operator==(const TDataLoop<T> & rhs) const;
  
  /**
   * \brief Overloaded operator+= to add a value to the end of this dataloop
   *
   * \detail This operator takes an template reference, creates a new _Node to hold it, and inserts that at the end of this dataloop. In other words, the new node will be immediately before the start node, but the start is not changed. 
   *
   * \param[in] num A constant reference to an template value
   *
   * \return A reference to this updated DataLoop object with the new value added
   */
  TDataLoop & operator+=(const T & num);
   ...
   */
  TDataLoop operator+(const TDataLoop<T> & rhs) const;
   ...
   * \return A reference to the updated DataLoop object 
   */
  TDataLoop & splice(TDataLoop<T> & rhs, size_t pos);
   ...
   */
  template <typename U> friend std::ostream & operator<<(std::ostream & os, const TDataLoop<U> & dl);
